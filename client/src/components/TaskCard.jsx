...
const TaskCard = ({ task, getUserById, setTasks }) => {
  ...
  const deleteHandler = async (id) => {
    try {
      console.log(id);
      const response = await axios.delete(`http://10.66.67.132:8000/api/tasks/${id}`)
      console.log("button")
      ...
      <div className='taskcard-outer-div'>
        ...
        <div className='taskcard-bimsgdetail-outer-1'>
          <div className='taskcard-bimsgdetail-outer-2'>
            ...
            <div className='mdattachfile' onClick={() => task?.assets?.length > 0 && window.open(`http://10.66.67.132:8000/api/tasks/assets/${task._id}/${task?.assets[0]}`)}>
              <MdAttachFile />
